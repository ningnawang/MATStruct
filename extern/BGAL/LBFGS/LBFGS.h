...
class _LBFGS {
 ...
 private:
  ...
  template <class fun>
  int linear_searchI_(fun& f, double& fval, Eigen::VectorXd& iterX,
                      Eigen::VectorXd& gradient, double& step,
...
inline _LBFGS::_Parameter::_Parameter() {
  rpd_itr = 0;
  k_itr = 0;
  m = 6;
  max_iteration = 1000;
...
inline _LBFGS::_Parameter::_Parameter(const _Parameter& in_parameter) {
  rpd_itr = in_parameter.rpd_itr;
  m = in_parameter.m;
...
inline _LBFGS::_LBFGS() {
  //_parameter = _Parameter();
}

inline _LBFGS::_LBFGS(const _Parameter& in_parameter)
...
int _LBFGS::minimize(fun& f, Eigen::VectorXd& iterX) {
  ...
  double fval = f(iterX, gradient, _parameter.rpd_itr);
...
int _LBFGS::minimizeI(fun& f, Eigen::VectorXd& iterX, int IterNum) {
  ...
  while (1) {
    ...
    oldGradient = gradient;
    int num_linear =
        linear_searchI_(f, fval, iterX, gradient, step, direction, IterNum);
    if (num_linear == _parameter.max_linearsearch) {
      if (_parameter.is_show) {
        std::cout << "reach the max linearsearch time" << std::endl;
      }
...
template <class fun>
int _LBFGS::linear_searchI_(fun& f, double& fval, Eigen::VectorXd& iterX,
                            Eigen::VectorXd& gradient, double& step,
                            const Eigen::VectorXd& direction, int IterNum) {
  if (step < 0) {
...
int _LBFGS::linear_search_(fun& f, double& fval, Eigen::VectorXd& iterX,
  ...
  while (1) {
    iterX = iX + step * direction;
    fval = f(iterX, gradient, _parameter.rpd_itr);
    if (fval > ifval) {
      step_u = step;
    } else {
      double dg = gradient.dot(direction);
